# tests/test_integration_vlmodel.py
import pytest
import torch
...
# MockBatchEncoding that supports .to(device)
...
# Fixture to patch model and processor globally for integration tests
...
def patch_model(monkeypatch):
    ...
    class DummyModel:
        def __init__(self):
            # Processor returns a MockBatchEncoding
            self.processor = MagicMock(
                return_value=MockBatchEncoding(
                    {
    ...
    monkeypatch.setattr(vl, "model", DummyModel())
    monkeypatch.setattr(vl, "id_to_label", {0: "label_0", 1: "label_1"})
...
def test_predict_text_raw_image_gcs(mock_storage_client, client):
    ...
    payload = {"text_raw": DUMMY_TEXT, "image_gcs": "image.jpg"}
...
# Test: text_gcs + image_base64
@patch("api.vlmodel_router.storage.Client")
def test_predict_text_gcs_image_base64(mock_storage_client, client):
    # Mock GCS text download
    mock_blob = MagicMock()
    mock_blob.exists.return_value = True
...
# Test: missing GCS blob raises 404
@patch("api.vlmodel_router.storage.Client")
