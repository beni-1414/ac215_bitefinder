import pytest
import torch
from unittest.mock import MagicMock, patch
from fastapi import FastAPI
from fastapi.testclient import TestClient
from api.vlmodel_router import router, InferenceRequest
import base64

DUMMY_IMAGE_BASE64 = """
"
"""
DUMMY_TEXT = "I am feeling itchy and have red spots all over."


# Mock BatchEncoding for processor output
class MockBatchEncoding(dict):
    def to(self, device):
        out = {}
        for k, v in self.items():
            out[k] = v.to(device) if hasattr(v, "to") else v
        return MockBatchEncoding(out)


# Mock model fixture
@pytest.fixture
def mock_model(monkeypatch):
    import api.vlmodel_router as vl

    processed = MockBatchEncoding(
        {
            "input_ids": torch.randn(1, 5),
            "attention_mask": torch.ones(1, 5),
            "pixel_values": torch.randn(1, 3, 32, 32),
        }
    )
    fake_logits = torch.tensor([[0.1, 0.2, 0.3, 0.4, 0.0, 0.0, 0.0]])
    fake_output = {"logits": fake_logits}

    mock_model = MagicMock()
    mock_model.processor = MagicMock(return_value=processed)
    mock_model.return_value = fake_output

    monkeypatch.setattr(vl, "model", mock_model)
    monkeypatch.setattr(vl, "id_to_label", {i: f"label_{i}" for i in range(7)})
    monkeypatch.setattr(vl, "device", "cpu")

    return mock_model


# Test client fixture
@pytest.fixture
def client(mock_model):
    app = FastAPI()
    app.include_router(router)
    return TestClient(app)


# Test successful prediction (text_raw + image_base64)
def test_predict_success(client, mock_model):
    payload = {"text_raw": DUMMY_TEXT, "image_base64": DUMMY_IMAGE_BASE64}
    res = client.post("/predict", json=payload)

    assert res.status_code == 200
    json_data = res.json()
    assert "prediction" in json_data
    assert "confidence" in json_data
    assert "probabilities" in json_data

    mock_model.processor.assert_called_once()
    mock_model.assert_called_once()


# Test input validation (both text inputs)
def test_inference_request_both_texts_error():
    with pytest.raises(ValueError):
        InferenceRequest(text_raw="a", text_gcs="b", image_base64="c")


# Test input validation (both image inputs)
def test_inference_request_both_images_error():
    with pytest.raises(ValueError):
        InferenceRequest(image_base64="a", image_gcs="b", text_raw="c")


# Test GCS text input path
@patch("api.vlmodel_router.storage.Client")
def test_predict_text_gcs(mock_storage_client, client, mock_model):
    # Mock bucket + blob
    mock_blob = MagicMock()
    mock_blob.exists.return_value = True
    mock_blob.download_as_text.return_value = "Text from GCS"
    mock_bucket = MagicMock()
    mock_bucket.blob.return_value = mock_blob
    mock_storage_client.return_value.bucket.return_value = mock_bucket

    payload = {"text_gcs": "input.txt", "image_base64": DUMMY_IMAGE_BASE64}
    res = client.post("/predict", json=payload)

    assert res.status_code == 200
    mock_blob.download_as_text.assert_called_once()


# Test GCS image input path
@patch("api.vlmodel_router.storage.Client")
def test_predict_image_gcs(mock_storage_client, client, mock_model):
    # Mock bucket + blob
    mock_blob = MagicMock()
    mock_blob.exists.return_value = True
    mock_blob.download_as_bytes.return_value = base64.b64decode(DUMMY_IMAGE_BASE64)
    mock_bucket = MagicMock()
    mock_bucket.blob.return_value = mock_blob
    mock_storage_client.return_value.bucket.return_value = mock_bucket

    payload = {"text_raw": DUMMY_TEXT, "image_gcs": "input.jpg"}
    res = client.post("/predict", json=payload)

    assert res.status_code == 200
    mock_blob.download_as_bytes.assert_called_once()


# Test missing GCS blob raises HTTPException
@patch("api.vlmodel_router.storage.Client")
def test_predict_missing_gcs_blob(mock_storage_client, client):
    mock_blob = MagicMock()
    mock_blob.exists.return_value = False
    mock_bucket = MagicMock()
    mock_bucket.blob.return_value = mock_blob
    mock_storage_client.return_value.bucket.return_value = mock_bucket

    # Text GCS missing
    payload = {"text_gcs": "missing.txt", "image_base64": DUMMY_IMAGE_BASE64}
    res = client.post("/predict", json=payload)
    assert res.status_code == 404

    # Image GCS missing
    payload = {"text_raw": DUMMY_TEXT, "image_gcs": "missing.jpg"}
    res = client.post("/predict", json=payload)
    assert res.status_code == 404


def test_load_model(monkeypatch):
    from api import vlmodel_router as vl

    # Mock get_secret
    monkeypatch.setattr(vl, "get_secret", lambda key: "fake_wandb_key")

    # Mock W&B env vars
    monkeypatch.setenv("WANDB_TEAM", "team")
    monkeypatch.setenv("WANDB_PROJECT", "project")

    # Mock wandb.Api
    fake_artifact = MagicMock()
    fake_artifact.download.return_value = "/fake/path"
    fake_artifact.metadata = {"model_id": "dummy_model", "model_kwargs": {"num_labels": 3}, "id_to_label": {"0": "A", "1": "B", "2": "C"}}
    fake_api = MagicMock()
    fake_api.artifact.return_value = fake_artifact
    monkeypatch.setattr(vl.wandb, "Api", lambda: fake_api)

    # Mock model
    class DummyModel:
        def __init__(self, num_labels):
            self.model = MagicMock()
            self.num_labels = num_labels
            self.processor = MagicMock()
            self.classifier = MagicMock()

        def eval(self):
            pass

    monkeypatch.setitem(vl.model_classes, "dummy_model", DummyModel)

    # --- Mock torch.load to avoid file I/O ---
    monkeypatch.setattr(vl.torch, "load", lambda path, map_location: {})

    # Call load_model
    vl.load_model()

    # Assertions
    assert vl.model is not None
    assert isinstance(vl.model, DummyModel)
    assert vl.model.num_labels == 3
    assert vl.id_to_label == {0: "A", 1: "B", 2: "C"}
