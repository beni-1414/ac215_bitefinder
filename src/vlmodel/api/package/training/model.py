...
class CLIPForBugBiteClassification(nn.Module):
    def __init__(self, num_labels, pretrained='openai/clip-vit-base-patch32', freeze_params=True, dropout_prob=0.1):
        ...
        # Classification head (linear layer) to project image+text embeddings (image_embeds+text_embeds=projection_dim*2) to label space
        self.classifier = nn.Sequential(nn.Dropout(dropout_prob), nn.Linear(2 * self.model.config.projection_dim, num_labels))
        ...
        if freeze_params:  # Freeze pre-trained model parameters
            for name, param in self.model.named_parameters():
                if not any(x in name for x in ['classifier', 'visual_projection', 'text_projection']):
...
class ViLTForBugBiteClassification(nn.Module):
    def __init__(self, num_labels, pretrained='dandelin/vilt-b32-mlm', freeze_params=True, dropout_prob=0.1):
        ...
        # Classification head (linear layer) to project encoding space (of hidden_size dimensionality) to label space
        self.classifier = nn.Sequential(nn.Dropout(dropout_prob), nn.Linear(self.model.config.hidden_size, num_labels))
        if freeze_params:  # Freeze pre-trained model parameters
