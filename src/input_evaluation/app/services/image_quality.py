...
def decide(metrics: IQAMetrics) -> Tuple[bool, str, str]:
    ...
    if metrics.blur_laplacian_var < t.MIN_LAPLACIAN_VAR:
        return (
            False,
            "Image looks out of focus. Hold steady and tap to focus on the bite.",
            "blur_laplacian_var",
        )
    ...
    if (
        ...
        return (
            ...
            "exposure_hist_entropy",
        )
    if metrics.noise_estimate_sigma > t.MAX_NOISE_SIGMA:
        return (
            False,
            "Image is noisy. Improve lighting or avoid digital zoom.",
            "noise_estimate_sigma",
        )
